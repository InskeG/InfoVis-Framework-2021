 ...
 * BASE CODE TAKEN FROM AND BASED ON https://npmjs.org/package/timelines-chart
 */
...
(function (global, factory) {
  ...
  function d3Tip() {
    ...
    //
    // Returns a tip
    tip.show = function(d=null) {
      if (d == null || !d.target.classList.contains('disabled')) {
  ...
  var DiscreteLegend = index({
    ...
    update: function update(state) {
      let filter = null;
      if (!d3.select(`.series-segment.disabled`).empty()) {
        filter = d3.select(`.series-segment:not(.disabled)`).attr('class').split(' ').pop();
      }
  ...
  var timelines = index({
    ...
    init: function init(el, state) {
      ...
      function buildDomStructure() {
        ...
        state.colorLegend = legend().onLegendClick(state.onLegendClick)(state.svg.append('g').attr('class', 'legendG').node());
      ...
      function addTooltips() {
        ...
        state.segmentTooltip = d3Tip().attr('class', 'chart-tooltip segment-tooltip').direction('s').offset([5, 0]).html(function (event, d) {
          ...
          return '<strong>' + d.labelVal + ' </strong>' + state.zDataLabel + (normVal ? ' (<strong>' + Math.round((d.val - state.zColorScale.domain()[0]) / normVal * 100 * 100) / 100 + '%</strong>)' : '') + '<br>' + dateFormat(d.timeRange[0]);
        });
    ...
    update: function update(state) {
      ...
      function renderGroups() {
        ...
        var newGroups = groups.enter().append('rect')
            ...
            .on('mouseover', () => { if (state.showGroupTooltip) state.groupTooltip.show })
            .on('mouseout', () => { if (state.showGroupTooltip) state.groupTooltip.hide });
      ...
      function renderTimelines(maxElems) {
        ...
        var newSegments = timelines.enter().append('rect')
            ...
            .on('mouseover.groupTooltip', state.showGroupTooltip ? state.groupTooltip.show : null)
            .on('mouseout.groupTooltip', state.showGroupTooltip ? state.groupTooltip.hide : null)
            .on('mouseover.lineTooltip', state.showLineTooltip ? state.lineTooltip.show : null)
            .on('mouseout.lineTooltip', state.showLineTooltip ? state.lineTooltip.hide : null)
            .on('mouseover.segmentTooltip', state.segmentTooltip.show)
            .on('mouseout.segmentTooltip', state.segmentTooltip.hide);

        newSegments
            .on('mouseover', function (d) {
              if (('disableHover' in state && state.disableHover) || d.target.classList.contains('disabled')) return;

              moveToFront()(this);

              var hoverEnlarge = state.lineHeight * hoverEnlargeRatio;

              d3Select(this).transition().duration(70)
                  .attr('x', (d) => state.xScale(d.timeRange[0]) - hoverEnlarge / 2)
                  .attr('width', (d) => max([1, state.xScale(d.timeRange[1]) - state.xScale(d.timeRange[0])]) + hoverEnlarge)
                  .attr('y', (d) => state.yScale(d.group + '+&+' + d.label) - (state.lineHeight + hoverEnlarge) / 2)
            ...
            .on('mouseout', function (d) {
              ...
              d3Select(this).transition().duration(250)
                  .attr('x', (d) => state.xScale(d.timeRange[0]))
                  .attr('width', (d) => max([1, state.xScale(d.timeRange[1]) - state.xScale(d.timeRange[0])]))
                  .attr('y', (d) => state.yScale(d.group + '+&+' + d.label) - state.lineHeight / 2)
        ...
        timelines.transition().duration(state.transDuration)
            .attr('x', (d) => { return state.xScale(d.timeRange[0]) })
            .attr('width', (d) => { return max([1, state.xScale(d.timeRange[1]) - state.xScale(d.timeRange[0])]) })
            .attr('y', (d) => { return state.yScale(d.group + '+&+' + d.label) - state.lineHeight / 2 })
            .attr('height', state.lineHeight)
